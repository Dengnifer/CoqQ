...
pose f (c : 'I_n -> R) (i : nat) : R := match (i < n)%N =P true with
                    ...
                    | _ => 0 end.
have mf c : measurable_fun setT (f c) by [].
have := minkowski_EFin [the measure _ _ of counting] (mf a) (mf b) p1.
rewrite !Lnorm_counting//.
rewrite (nneseries_split 0 n); last by move=> k; rewrite lee_fin powR_ge0.
rewrite add0n ereal_series_cond eseries0 ?adde0; last first.
...
Proof. by rewrite qualifE /nneg_num_pred lpnormrc_ge0. Qed.
...
apply: (ler0_derive1_le_cc (f := fun p => p^-1 *
...
- by rewrite in_itv/= lexx P12.
- by rewrite in_itv/= P12 lexx.
Qed.
...
have P3 (p : R) : 0 < p -> is_derive p 1
  ...
  by apply: is_derive_sum=>/= i _; apply/is_derive1_powRx.
  by rewrite addrC /GRing.scale/=; f_equal; rewrite ?mulrA// mulrC.
apply: (ger0_derive1_ndecr (f := fun p => p^-1 *
...
Proof. by move=>/chain_homo P i j Pij; move: (P _ _ Pij); rewrite/= -Order.le_val. Qed.

Lemma Dge0_subproof : forall (x : D), Denmx0 ⊑ x.
Proof. by move=>x/=; case: x=>m Pm; rewrite -Order.le_val/= le_lownerE subr0; apply/denmx_psd. Qed.
...
by move=>x Px; rewrite -Order.le_val/=; apply: cmxlimn_le.
Qed.
