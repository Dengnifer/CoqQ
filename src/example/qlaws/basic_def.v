...
HB.lock
Definition onb_change {R : realType} (U V : @chsType R) (F : finType) (u : F -> U) 
  (v : F -> V) := \sum_i [> v i; u i <].

Section BasicDefReal.
...
Qed.
HB.instance Definition _ F (U : chsType) b :=
  isQMeasure.Build R U U F (@onb_meas F U b) 
  (onb_meas_tp b).
...
Lemma onb_change_adj U V F u v :
  (@onb_change R U V F u v)^A = @onb_change R V U F v u.
...
Lemma onb_change_bound1 U V F (u : 'PONB) (v : 'PONB) :
  @onb_change R U V F u v \is bound1lf.
Proof.
...
HB.instance Definition _ U V F (u : 'PONB(F;U)) (v : 'PONB(F;V)) 
  := isBound1Lf.Build R U V (@onb_change R U V F u v) (onb_change_bound1 u v).
Lemma onb_change_iso U V F (u : 'ONB) (v : 'PONB) :
  @onb_change R U V F u v \is isolf.
Proof.
...
HB.instance Definition _ U V F (u : 'ONB(F;U)) (v : 'PONB(F;V)) := 
  Bound1_isIsoLf.Build R U V (@onb_change R U V F u v) (onb_change_iso u v).
Lemma onb_change_giso U V F (u : 'ONB) (v : 'ONB) :
  @onb_change R U V F u v \is gisolf.
Proof.
...
HB.instance Definition _ U V F (u : 'ONB(F;U)) (v : 'ONB(F;V)) := 
  Iso_isGisoLf.Build R U V (@onb_change R U V F u v) (onb_change_giso u v).
Lemma onb_changeE U V F (u : 'ONB) v i : (@onb_change R U V F u v) (u i) = v i.
Proof.
...
HB.instance Definition _ U (u : 'ONB(bool; U)) := 
  isONB.Build R U bool (@onb_swap U u) (@onb_swap_onb U u) (@onb_card R U bool u).
Lemma onb_swapK U u : onb_swap (@onb_swap U u) = u.
Proof. by apply/funext; case. Qed.
Lemma onb_change_swap (U : chsType) (u : bool -> U) :
  @onb_change R U U bool u (onb_swap u) = [> u true; u false <] + [> u false; u true <].
...
HB.instance Definition _ T (phi : 'NS('Hs T)) := 
  isQMeasure.Build R _ _ _ (bvmeas phi) (bvmeas_qm phi).
...
Proof. by move: (@is_trnincr R U V bool M); rewrite /trace_nincr !big_bool/= addrC. Qed.
HB.instance Definition _ U V (M : 'TN(bool; U,V)) :=
  isTraceNincr.Build R U V bool (M ^m⟂) (bcmeas_tn M).
Lemma bcmeas_tp U V (M : 'QM(bool; U,V)) : trace_presv (M ^m⟂).
Proof. by move: (@is_trpresv R U V bool M); rewrite /trace_presv !big_bool/= addrC. Qed.
HB.instance Definition _ U V (M : 'QM(bool; U,V)) :=
  TraceNincr_isQMeasure.Build R U V bool (M ^m⟂) (bcmeas_tp M).
...
HB.instance Definition _ (P : {hspace U}) := 
  isQMeasure.Build R _ _ bool (@pmeas P) (@pmeas_tp P).
...
HB.instance Definition _ U := isQMeasure.Build R _ _ _ (@measT U) (measT_qm U).
...
HB.instance Definition _ U := isQMeasure.Build R _ _ _ (@measF U) (measF_qm U).
...
exists (ONB.Pack (ONB.Class
  (@isFullDim.Axioms_ R U _ uf (@isPONB.Axioms_ R U _ uf Pu) Pd))).
exists (ONB.Pack (ONB.Class
  (@isFullDim.Axioms_ R U _ vf (@isPONB.Axioms_ R U _ vf Pv) Pd))).
...
Lemma big_bool_cond [T : Type] [idx : T] [op : Monoid.com_law idx] b (F : bool -> T) :
...
move: (IH x) (qm_trlf M ((N b2 \o x) \o (N b2)^A))=>/(f_equal (@lftrace R U)) + /eqP.
...
have PXY (X Y : {hspace U}): (X _C_ Y)%O -> ((X \o Y) \o (X \o Y)) = (X \o Y) \o X.
  ...
  set K := (X^⟂ + Y^⟂)^A.
  have Kcos : (((X \o Y) \o X) \o cosupplf K) \o K = ((X \o Y) \o X) \o K
    by rewrite -comp_lfunA (@cosupplfv R U U K).
  move=>H0.
  rewrite comp_lfun0l in H0.
  rewrite Kcos in H0.
  rewrite /K in H0.
  rewrite adjfD in H0.
  rewrite /cplmt in H0.
  rewrite adjfB adjfB in H0.
  move: H0.
...
HB.instance Definition _ U V W F (f : 'TN(F;U,V)) (u : 'FB1(W,U)) :=
  isTraceNincr.Build R _ _ F (measUr f u) (measUr_tn f u).
...
HB.instance Definition _ U V W F (f : 'QM(F;U,V)) (u : 'FI(W,U)) :=
  TraceNincr_isQMeasure.Build R _ _ F (measUr f u) (measUr_tp f u).
